    ...
    public class DisableItemInList : AbstractItemListCommand
    {
        ...
        public override bool ExecuteInternal(ListKey key, ListKey personalKey, ListKey serverKey, bool personalExists, bool serverExists, Dictionary<string, IParameter> parameters, SocketMessage messageArgs, SocketGuild server)
        {
            ...
            foreach (var item in items)
            {
                ...
                // updates the item
                Database.Instance.DB.UpdateItemInList(key, item, item, isEnabled: false);
            }

            SendMessage(messageArgs, $"All requested items ({rawItem}) have been disabled for randomization in the {(key.IsPersonal ? "personal" : "server-owned")} list with the name [{key.Name}]!");
