...
public class ToggleGameEnablement : AbstractCommand
{
    public ToggleGameEnablement() : base("toggle_game_enablement_in_gamelist", "Toggles a game between being valid for randomization or not in a game list")
    {
    }
    ...
    public override bool ExecuteInternal(Dictionary<string, string> args, SocketMessage messageArgs, SocketGuild server)
    {
        ...
        if (File.Exists(fileName))
        {
            ...
            else
            {
                ...
                {
                    games.Games[foundIndex].IsEnabled = !games.Games[foundIndex].IsEnabled;
                    File.WriteAllText(fileName, JsonConvert.SerializeObject(games));
                    SendMessage(messageArgs, $"The game {gamename} in the list named {listname} has been toggled to be {(games.Games[foundIndex].IsEnabled ? "Enabled" : "Disabled")} for randomization!");
                }
            }
        }
        else
