...
public class DisableAllGamesInGameList : AbstractCommand
{
    public DisableAllGamesInGameList() : base("disable_all_games_in_gamelist", "Disables all games from randomization in a game list")
    {
    }
    ...
    public override bool ExecuteInternal(Dictionary<string, string> args, SocketMessage messageArgs, SocketGuild server)
    {
        ...
        if (File.Exists(fileName))
        {
            ...
            else
            {
                ...
                {
                    games.Games[i].IsEnabled = false;
                }
                File.WriteAllText(fileName, JsonConvert.SerializeObject(games));
                SendMessage(messageArgs, $"All games in the list named {listname} have been disabled for randomization!");
            }
        }
        else
        {
